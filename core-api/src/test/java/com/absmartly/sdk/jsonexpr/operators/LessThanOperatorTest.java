...
class LessThanOperatorTest extends OperatorTest {
	final LessThanOperator operator = new LessThanOperator();

	@Test
	void testEvaluate() {
		assertFalse((Boolean) operator.evaluate(evaluator, listOf(0, 0)));
		...
		assertTrue((Boolean) operator.evaluate(evaluator, listOf(0, 1)));
		verify(evaluator, times(2)).evaluate(any());
		verify(evaluator, times(1)).evaluate(0);
		verify(evaluator, times(1)).evaluate(1);
